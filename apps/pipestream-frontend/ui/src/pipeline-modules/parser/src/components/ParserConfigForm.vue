...
export default {
  ...
  methods: {
    ...
    getApiUrl(path) {
      // If we're running through Traefik (port 38080), use full path
      if (window.location.port === '38080') {
        return `/modules/parser${path}`
      }
      // If running directly (port 39001), use path as-is
      return path
    },

    async loadModuleInfo() {
      try {
        const apiUrl = this.getApiUrl('/api/parser/service/info')
        console.log('Loading module info from', apiUrl)
    ...
    async loadSchema() {
      ...
      try {
        ...
        const apiUrl = this.getApiUrl('/api/parser/service/config')
        const response = await axios.get(apiUrl, {
    ...
    async submitForm() {
      try {
        ...
        const apiUrl = this.getApiUrl('/api/parser/service/parse-json')
        console.log('Sending JSON request to', apiUrl)
        console.log('JSON data:', JSON.stringify(jsonRequest, null, 2))

        const response = await axios.post(apiUrl, jsonRequest, {
          headers: {
    ...
    async loadDemoDocuments() {
      try {
        ...
        const apiUrl = this.getApiUrl('/api/parser/service/demo/documents')
        const response = await axios.get(apiUrl)
    ...
    async parseSelectedDemo() {
      ...
      try {
        ...
        const apiUrl = this.getApiUrl('/api/parser/service/demo/parse')
