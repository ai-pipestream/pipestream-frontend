...
const fetchDynamicLinks = async (): Promise<LinkItem[]> => {
  try {
    console.log('[SharedLinksPage] Fetching dynamic navigation items')
    ...
    console.warn('[SharedLinksPage] Failed to fetch dynamic links:', e)
    return []
  }
}
...
const refreshLinks = async (): Promise<void> => {
  ...
  try {
    ...
    console.log(`[SharedLinksPage] Loaded ${allLinks.value.length} links (${staticLinks.length} static, ${dynamicLinks.length} dynamic)`)
  } catch (e: any) {
